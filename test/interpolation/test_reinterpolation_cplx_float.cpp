...
public :
  ...
  void operator()(std::array<FLT,DIM>* X, int Nx, std::array<FLT,DIM>* Y, int Ny, CPLX* A){
	...
	if(std::isnan(A[j*Nx+i].real())){A[j*Nx+i] = CPLX(0.);}
      }
    }
  }
...
int main(int argc, char* argv[]){
  ...
  theia::get_M2M<DIM,FLT,CPLX,0,0>
    (LL, minsXu, maxsXu, LL, minsX, maxsX, M2M_x);
  theia::Kron<DIM,CPLX> M2M_y;
  theia::get_M2M<DIM,FLT,CPLX,0,0>
    (LL, minsYu, maxsYu, LL, minsY, maxsY, M2M_y);
  
  // M2L matrices
  CPLX *M2L = nullptr;
  theia::get_M2L<DIM,FLT,CPLX,light,0>(minsXu, maxsXu, L,
				       minsYu, maxsYu, L,
				       M2L   , Kernel);
  ...
  std::cout << std::boolalpha << (errmax < 1.e-5) << std::endl;
